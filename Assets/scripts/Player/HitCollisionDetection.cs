...
public class HitCollisionDetection : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.gameObject.tag == "Monster") 
        {
            ...
            else
            {
             ...
             StartCoroutine(MonsterDestroy(other.gameObject));
    ...
    public IEnumerator MonsterDestroy(GameObject other) 
    {
        ...
        GameObject puffEffectObj = Instantiate(puffEffect, other.transform.position, Quaternion.identity);
        Destroy(puffEffectObj, 0.1f);
